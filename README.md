...
→ [Australian Filmography](https://paulduchesne.github.io/australian-filmography/)
...
Data is collected from Wikidata using the following script which downloads data fragments as individual [JSON-LD](https://json-ld.org/) files. These are then aggregated into a single `wikidata.json` file.
...
If you wish to run a dev instance, use the following command from within the `docs` directory.

```sh
python3 -m http.server 8642
```

The service should then be available locally at http://localhost:8642/.
